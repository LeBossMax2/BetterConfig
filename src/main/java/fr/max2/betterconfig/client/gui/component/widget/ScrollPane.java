...
public class ScrollPane extends BCComponent<IScrollComponent> implements IScrollComponent, IComponentParent
{
	...
	protected void enableScissor(PoseStack matrixStack, int x, int y, int width, int height)
	{
		...
		Vector4f topLeft = new Vector4f(x, y, 0.0f, 1.0f).mul(mat);
		Vector4f size = new Vector4f(width, height, 0.0f, 0.0f).mul(mat);
